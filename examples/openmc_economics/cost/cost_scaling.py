...
from cost.cost_utils import update_high_level_costs, prettify, energy_cost_levelized
...
def cost_scale_general(db, account_number, old_var, new_var, scaling_exponent, old_cost, constant_cost):
    ...
    db.loc[db.Account == account_number, 'Estimated Cost (2024 $)'] =  new_cost      
...
def add_customized_cost_estimate(db, account_number, customized_cost_estimate):
    db.loc[db.Account == account_number, 'Estimated Cost (2024 $)'] =  customized_cost_estimate 
...
def cost_estimate(code_of_account , params) :
    power_kWe = 1000 * params['power_MW_th'] * params['thermal_efficiency'] 
      ...
      #Create new database
    cost_new_reactor = code_of_account[['Account', 'Account Title', 'Estimated Cost (2024 $)']].copy()  
    
    for account in [11]: 
        scale_unit_costs(cost_new_reactor, account, cost_dictionary_data['Land Unit Cost'], params['land_area_acres'])

    
    db_updated = update_high_level_costs(cost_new_reactor, power_kWe) 
    
    (db_updated.loc[db_updated.Account == 'Annualized Cost ($/MWh)', 'Estimated Cost (2024 $)'])   =\
        (db_updated.loc[db_updated.Account == 'Annualized Cost ($/ year)', 'Estimated Cost (2024 $)'])\
            .values[0] / (power_kWe/ 1000 * params['capacity_factor'] * 365 * 24)
        
    cap_cost = (db_updated.loc[db_updated.Account == 'Total Capital Investment (TCI)', 'Estimated Cost (2024 $)']).values[0]
    ann_cost = (db_updated.loc[db_updated.Account == 'Annualized Cost ($/ year)', 'Estimated Cost (2024 $)']).values[0]
    ann_cost_levelized = (db_updated.loc[db_updated.Account == 'Annualized Cost ($/MWh)', 'Estimated Cost (2024 $)']).values[0]
    ...
    (db_updated.loc[db_updated.Account == 'LCOE', 'Estimated Cost (2024 $)'])   = lcoe1 
